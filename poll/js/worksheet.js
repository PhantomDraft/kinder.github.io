...
/**
 * The Worksheet class manages task rendering,
 * applies input masks and performs answer validation.
 */
			...
			const addInput = () => {
				...
				if (Array.isArray(task.signs)) {
					task.signs.forEach(s => {
						...
						btn.textContent = s;
						// Prevent input from losing focus on mousedown so blur validation
						// doesn't trigger before we set the value via click
